...
def match_rotation(tagType, starting_or, starting_rot):
    # all actions under one transaction
    with revit.TransactionGroup("Match tag rotation"):
...
#        with EasilySelectableElements(revit.active_view, BIC.OST_Tags):
        ...
        for picked_element in revit.get_picked_elements_by_category(
            ...
            axisStart = (picked_element.Location.Point)
            axisEnd = XYZ(axisStart.X , axisStart.Y , axisStart.Z +100)
            
            axis = Line.CreateBound(axisStart, axisEnd);
            
            Location = picked_element.Location
			
            rot = starting_rot - picked_element.Location.Rotation
			
            #rot = (starting_rot +3.1415) % (2*3.1415)
            #UI.TaskDialog.Show('pyRevitPlus', str(starting_rot))
			
            with revit.Transaction("Setting tag \'{}\' rotation".format(picked_element.Id)):
                picked_element.TagOrientation = starting_or

                Location.Rotate(axis, rot)

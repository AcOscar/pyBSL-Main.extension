...
is_cloud = getattr(doc, "IsModelInCloud", False)
...
if is_cloud:
    ...
    if cloud_model_path:
        ...
        if not forge_id:
            output.print_md(
                "**Error:** Forge Project ID not configured."
            )
            exit
        else:
            output.print_md("**Forge Project Id**: {}".format(forge_id))

        if not item_urn:
            output.print_md(
                "**Error:** Item URN not configured."
            )
            exit
        else:
            output.print_md("**Item URN**: {}".format(item_urn))

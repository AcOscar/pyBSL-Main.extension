...
Reading sheet list from excel
"""

__title__ = "Excel BTK Reader 2"
...
def ensure_list(value):
    ...
    if isinstance(value, list):
        return value
    raise Exception("Unexpected type for SheetSync: {}.".format(type(value)))
...
def validate_config(cfg):
    ...
    ps_list = ensure_list(cfg.get("SheetSync"))
    for i, item in enumerate(ps_list):
        if not isinstance(item, dict):
            raise Exception("SheetSync[{}] is not an object.".format(i))
        validate_parameter_sync_item(item, "SheetSync[{}]".format(i))
...
def excel_date_to_string(cell):
    """Formats a cell's datetime value to a string matching the Excel format."""
    if isinstance(cell.value, datetime):
        # Retrieve the Excel number format string (e.g., 'dd-mmm-yy')
        excel_format = cell.number_format
        
        # NOTE: This is a simplified translation. 
        # Excel format codes are not identical to Python's strftime codes.
        # You may need a more comprehensive mapping for complex formats.
        
        # Basic mapping example:
        # Excel 'dd-mmm-yy' -> Python '%d-%b-%y'
        # Excel 'yyyy-mm-dd' -> Python '%Y-%m-%d'
        
        python_format = excel_format.replace('-','.').replace('yyyy', '%Y').replace('yy', '%Y').replace('mm', '%m').replace('dd', '%d').replace('mmm', '%b').replace('hh', '%H').replace('ss', '%S')

        try:
            return cell.value.strftime(python_format)
        except ValueError:
            # Fallback for complex formats not handled by simple replace
...
def set_parameter_value_safely(element, param_name, value):
    """
...
def sync_sheets(doc, sheets, excel_data_dict, key_param_name, output):
    """
    Sync sheets from Excel data to Revit sheets
    
    Args:
        doc: Revit document
        sheets (list): List of Revit sheets
    ...
    transaction = Transaction(doc, "Sync sheets from Excel")
    transaction.Start()
    ...
    try:
        for sheet in sheets:
            ...
            # Get the key parameter value from the sheet
            key_value = get_parameter_value_safely(sheet, key_param_name)
            
            if not key_value:
                output.print_md("  Sheet {} has no value for key parameter '{}'".format(
                    sheet.Id, key_param_name))
                stats['warnings'] += 1
                continue
            
            # Look up the sheet in Excel data
            ...
            output.print_md("  Processing sheet {} with id '{}'".format(key_value, sheet.Id ))
            
            # Update parameters for this sheet
            ...
            for param_name in available_params:
                ...
                success, message = set_parameter_value_safely(sheet, param_name, excel_value)
    ...
    try:
        for element in elements:
            ...
            for param_name in available_params:
                ...
                success, message = set_parameter_value_safely(element, param_name, excel_value)
...
def main():
    ...
    output.print_md("-" * 50)
    params = cfg["SheetSync"]
    
    if not params:
        output.print_md("Fallback - old config style" )
        # Projektparameter auslesen
        project_info = doc.ProjectInformation

        excel_filename = get_parameter_value_safely(project_info, 'ExcelReader-FileName')
        excel_rangename = get_parameter_value_safely(project_info, 'ExcelReader-DataName')
        excel_keyname = get_parameter_value_safely(project_info, 'ExcelReader-KeyName')
        
        params = [{
            "Order": "0",
            "FileName": excel_filename,
            "DataName": excel_rangename,
            "KeyName": excel_keyname,
            "BuiltInCategory": "OST_Sheets",  # not important
            "Type": "Instance"  # not important
        }]
    ...
    for item in sorted(params, key=order_key):
        ...
        try:
            ...
            output.print_md("Starting sheet synchronization...")
            sync_sheets(doc, sheets, excel_lookup, excel_keyname, output)
